...
class PolaroidPoiSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ConstrainedBox(
      constraints: BoxConstraints(maxWidth: 390, maxHeight: media.size.height * maxHeightFactor),
...
class PolaroidPoiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if ((effectiveImageUrl ?? '').trim().isEmpty) {
      ...
      if (image is Map) {
        final value = (image['url'] ?? image['downloadUrl'] ?? '').toString().trim();
            ...
            child: IgnorePointer(
              child: Center(
                child: Transform.rotate(
                  angle: -.035,
                  child: CustomPaint(size: const Size(112, 34), painter: _TapePainter()),
                ),
              ),
            ),
          ),
          Positioned(
            ...
            child: Material(
              ...
              child: InkWell(
                ...
                child: const SizedBox(width: 36, height: 36, child: Icon(Icons.close_rounded, size: 22)),
              ),
            ),
          ),
        ],
...
class _PhotoAreaState extends State<_PhotoArea> {
  ...
  Future<void> _resolve(String? raw) async {
    final url = (raw ?? '').trim();
    if (url.isEmpty || url.startsWith('http://') || url.startsWith('https://') || url.startsWith('assets/') || url.startsWith('/assets/')) {
...
class _InfoArea extends StatelessWidget {
  ...
  String _prettyHours(String raw) {
    ...
    try {
      ...
      if (decoded is Map) return decoded.entries.take(3).map((e) => '${e.key}: ${e.value}').join(' · ');
  ...
  Future<void> _launch(BuildContext context, Uri uri) async {
    ...
    if (!await canLaunchUrl(uri)) {
      messenger.showSnackBar(const SnackBar(content: Text('Impossible d’ouvrir cette action.')));
      return;
    }
  ...
  Future<void> _directions(BuildContext context) async {
    ...
    await _launch(context, Uri.parse('https://www.google.com/maps/search/?api=1&query=$lat,$lng'));
  }
          ...
          children: [
            Expanded(
              child: Text(
                ...
                style: const TextStyle(fontFamily: 'MASLIVEBrushV2', fontSize: 30, height: 1, color: Color(0xFF161616)),
              ),
            ),
            const Icon(Icons.favorite_border_rounded, size: 26),
          ],
        ),
        Container(height: 1, margin: const EdgeInsets.only(top: 5, bottom: 9), color: Colors.black.withValues(alpha: .24)),
        if (descriptionValue.isNotEmpty) ...[
          Text(descriptionValue, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontFamily: 'MASLIVEBrushV2', fontSize: 18, height: 1.05)),
          const SizedBox(height: 8),
        ],
        _InfoRow(icon: Icons.schedule_rounded, text: hoursValue.isEmpty ? 'Horaires non renseignés' : hoursValue, muted: hoursValue.isEmpty),
        const SizedBox(height: 5),
        _InfoRow(icon: Icons.phone_rounded, text: phoneValue.isEmpty ? 'Téléphone non renseigné' : phoneValue, muted: phoneValue.isEmpty),
        if ((address ?? '').trim().isNotEmpty) ...[
          const SizedBox(height: 5),
          _InfoRow(icon: Icons.place_rounded, text: address!.trim()),
        ],
        LiveTableStatusSection(meta: meta, countryId: countryId, eventId: eventId, circuitId: circuitId, poiId: poiId),
        const SizedBox(height: 13),
        Row(
          children: [
            Expanded(
              child: _BrushButton(
                ...
                label: 'APPELER',
                onPressed: cleanedPhone == null ? null : () => _launch(context, Uri(scheme: 'tel', path: cleanedPhone)),
              ),
            ),
            ...
            Expanded(
              child: _BrushButton(
                ...
                label: 'ITINÉRAIRE',
                onPressed: (mapsUrl ?? '').trim().isNotEmpty || (lat != null && lng != null) ? () => _directions(context) : null,
              ),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        ...
        Expanded(child: Text(text, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: 'MASLIVEBrushV2', fontSize: 16, height: 1.05, color: color))),
      ],
    );
  }
}
...
class _BrushButton extends StatelessWidget {
  const _BrushButton({required this.icon, required this.label, required this.onPressed});
  final IconData icon;
  final String label;
          ...
          TextButton.icon(
            ...
            label: Row(
              ...
              children: [
                ...
                const SizedBox(width: 7),
                Flexible(child: Text(label, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 12))),
              ],
            ),
          ),
        ],
...
class _BrushStrokePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = const Color(0xFFF39AB7).withValues(alpha: enabled ? .9 : .35);
    final path = Path()
      ..moveTo(2, size.height * .28)
      ..quadraticBezierTo(size.width * .18, 1, size.width * .43, size.height * .16)
      ..quadraticBezierTo(size.width * .72, 2, size.width - 2, size.height * .24)
      ..lineTo(size.width - 5, size.height * .76)
      ..quadraticBezierTo(size.width * .68, size.height, size.width * .42, size.height * .86)
  ...
  @override
  bool shouldRepaint(covariant _BrushStrokePainter oldDelegate) => oldDelegate.enabled != enabled;
}
...
class _TapePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final path = Path()
      ...
      ..close();
    canvas.drawPath(path, Paint()..color = const Color(0xFFE7D29E).withValues(alpha: .86));
    final line = Paint()..color = Colors.white.withValues(alpha: .18)..strokeWidth = 1;
