...
class GroupTrackingService {
  ...
  bool _isUsable(GpsSample sample) {
    if (!sample.lat.isFinite || !sample.lng.isFinite) {
      return false;
    }
    if (sample.lat < -90 || sample.lat > 90) {
      return false;
    }
    if (sample.lng < -180 || sample.lng > 180) {
      return false;
    }
  ...
  Future<void> stopTracking({required String projectId}) async {
    ...
    try {
      ...
      // Nettoyage idempotent : le document peut déjà avoir expiré.
    }
  }
