...
const MASTER_ROLES = new Set(["admin", "admin_master", "superadmin", "super-admin"]);
const GROUP_ROLES = new Set(["group", "group-admin", "admin_group", "admin_groupe"]);
...
function hasCreatorActivity(userData) {
  const activities = Array.isArray(userData?.activities)
    ...
    : [];
  return activities.includes("createur_digital") || activities.includes("creator_digital");
}
...
async function authorizeDeclaredOwnerRole({ db, uid, ownerRole }) {
  ...
  if (declaredRole === "compte_pro") {
    ...
    return business.exists
      ...
      : readinessFailure("business_profile_missing", "Créez votre compte professionnel avant de publier.", "/business");
  }
  ...
  if (declaredRole === "superadmin") {
    const isAuthorized = userData.isAdmin === true || MASTER_ROLES.has(authoritativeRole);
    return isAuthorized
      ? { ready: true, code: "ready", message: "", actionRoute: null }
      : readinessFailure("seller_role_mismatch", "Le rôle vendeur déclaré ne correspond pas à votre profil.", "/account");
  }

  if (declaredRole === "admin_groupe") {
    return GROUP_ROLES.has(authoritativeRole)
      ? { ready: true, code: "ready", message: "", actionRoute: null }
      : readinessFailure("seller_role_mismatch", "Le rôle vendeur déclaré ne correspond pas à votre profil groupe.", "/group-admin");
  }

  if (declaredRole === "createur_digital") {
    return hasCreatorActivity(userData)
      ? { ready: true, code: "ready", message: "", actionRoute: null }
      : readinessFailure("seller_role_mismatch", "Activez votre profil créateur avant de publier.", "/media-marketplace");
  }

  return readinessFailure("seller_role_not_allowed", "Votre profil n’autorise pas la publication de produits vendables.", "/account");
}

async function evaluateSellerReadiness({ db, uid, ownerRole }) {
  const roleAuthorization = await authorizeDeclaredOwnerRole({ db, uid, ownerRole });
  ...
  switch (normalizeLower(ownerRole)) {
    ...
    case "compte_pro": {
      ...
      if (!data) {
        return readinessFailure("business_profile_missing", "Créez votre compte professionnel et renseignez votre identité/SIRET avant de publier.", "/business");
      }
      ...
      if (!siret || (status !== "approved" && status !== "active")) {
        return readinessFailure("business_verification_required", "Votre identité et votre SIRET doivent être validés avant la publication.", "/business");
      }
      if (!isStripePayable(data.stripe)) {
        return readinessFailure("stripe_connect_not_payable", "Finalisez Stripe Connect : le compte doit autoriser les encaissements et les virements avant de publier.", "/business");
      }
      return { ready: true, code: "ready", message: "", actionRoute: null };
    }

    case "createur_digital": {
      const query = await db.collection("photographers").where("ownerUid", "==", uid).limit(1).get();
      if (query.empty) {
        return readinessFailure("photographer_profile_missing", "Créez et faites valider votre profil créateur avant de publier.", "/media-marketplace");
      }
      const data = query.docs[0].data() || {};
      if (normalizeLower(data.status || "pending") !== "approved") {
        return readinessFailure("photographer_verification_required", "Votre profil créateur doit être validé avant la publication.", "/media-marketplace");
      }
      if (!isStripePayable(data.stripe)) {
        return readinessFailure("stripe_connect_not_payable", "Finalisez Stripe Connect : les encaissements et les virements doivent être actifs avant de publier.", "/media-marketplace");
      }
      return { ready: true, code: "ready", message: "", actionRoute: null };
    }
...
function validateSubmissionPayload(HttpsError, submission) {
  ...
  if (!normalizeString(submission.countryId) || !normalizeString(submission.eventId) || !normalizeString(submission.circuitId)) {
    throw new HttpsError("failed-precondition", "Pays, événement et circuit sont obligatoires pour un média.", {
  ...
  if (!Number.isFinite(price) || price <= 0) {
    throw new HttpsError("failed-precondition", "Le prix du média doit être supérieur à 0.", {
...
function createCommerceSubmissionHandlers({ admin, db, onCall, HttpsError }) {
  async function submitCommerceForReviewImpl({ uid, submissionId }) {
    ...
    if (!["draft", "rejected", "pending"].includes(submission.status)) {
    ...
    const readiness = await evaluateSellerReadiness({ db, uid, ownerRole: submission.ownerRole });
    if (!readiness.ready) throwBusinessError(HttpsError, readiness);

    if (submission.status === "pending") {
      return { success: true, status: "pending", idempotent: true, readinessCode: readiness.code };
    }
    ...
    return db.runTransaction(async (transaction) => {
      ...
      if (fresh.status === "pending") {
        return { success: true, status: "pending", idempotent: true, readinessCode: "ready" };
      }
      if (!["draft", "rejected"].includes(fresh.status)) {
        throw new HttpsError("failed-precondition", "Invalid submission status", {
          code: "submission_status_invalid",
      ...
      return { success: true, status: "pending", idempotent: false, readinessCode: "ready" };
    });
  }

  return {
    submitCommerceForReview: onCall(
      { region: "us-east1", cpu: 0.083, memory: "256MiB", timeoutSeconds: 30, maxInstances: 10 },
